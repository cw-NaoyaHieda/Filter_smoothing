...
{
	printf("Iteration %d:\n", k);
	printf("  fx = %f, beta = %f, q = %f, rho = %f, X_0 = %f\n", fx, sig(x[0]), pnorm(q_qnorm, 0, 1), sig(x[1]), X_0);
	...
	*/
	...
	for (s = 0; s < S; s++) {
		...
		while (grad_stop_check < 50 && (norm > 0.001)) {
			particle_filter(sig(x[0]), q_qnorm, sig(x[1]), X_0, filter_X_mean, predict_Y_mean);
			...
			printf("%d,%d,%f,%f\n", s, grad_stop_check, sig(x[0]), sig(x[1]));
			fprintf(fp, "%d,%d,%f,%f\n", s, grad_stop_check, sig(x[0]), sig(x[1]));
			grad_stop_check += 1;
			norm = sqrt(pow(sig(x[0]) - beta_est_pre, 2) + pow(sig(x[1]) - rho_est_pre, 2));// pow(pnorm(x[1], 0, 1) - q_qnorm_est_pre, 2)) + pow(sig(x[2]) - rho_est_pre, 2) + pow(x[3] - X_0_est_pre, 2));
			beta_est_pre = sig(x[0]);
			//q_qnorm_est_pre = pnorm(x[1], 0, 1);
			rho_est_pre = sig(x[1]);
			//X_0_est_pre = x[3];
