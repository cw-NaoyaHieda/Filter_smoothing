...
)
{
	...
	fx = -Q(beta, x[0], rho, X_0);
	//g[0] = -Q_grad_beta(sig(x[0]), q_qnorm, rho, X_0);
	g[0] = -Q_grad_q_qnorm(beta, x[0], rho, X_0);
...
int main(void) {
	...
	*/
	...
	for (s = 0; s < S; s++) {
		...
		grad_stop_check = 1;
		norm = 100;
		while (grad_stop_check < 50 && (norm > 0.0001)) {
			particle_filter(beta, x[0], rho, X_0, filter_X_mean, predict_Y_mean);
			...
			printf("%d,%d,%f,%f\n", s, grad_stop_check, pnorm(x[0],0,1));
			fprintf(fp, "%d,%d,%f,%f\n", s, grad_stop_check, pnorm(x[0],0,1));
			grad_stop_check += 1;
			norm = sqrt(pow(pnorm(x[0], 0, 1) - q_qnorm_est_pre, 2));//sqrt(pow(sig(x[0]) - beta_est_pre, 2) + pow(pnorm(x[1], 0, 1) - q_qnorm_est_pre, 2)) + pow(sig(x[2]) - rho_est_pre, 2) + pow(x[3] - X_0_est_pre, 2));
			//beta_est_pre = sig(x[0]);
			q_qnorm_est_pre = pnorm(x[0], 0, 1);
			//rho_est_pre = sig(x[1]);
			//X_0_est_pre = x[3];
