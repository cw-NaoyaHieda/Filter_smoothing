...
```{r}
data <- read.csv("Project1/Project1/check_hull_grad_only.csv",header=TRUE)
summary(data)

data<-cbind(data,number=c(rep(1,99) %>% as.factor()))
...
```{r,eval=FALSE}
data <- read.csv("check_hull_grad_only.csv",header=TRUE)
summary(data)
